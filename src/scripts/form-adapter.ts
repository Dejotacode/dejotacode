...
const responseError = (
  ...
  return payload.error?.message ??
    "Não foi possível enviar. Revise os campos e tente novamente.";
};
...
export const preparePublicForm = (selector: string) => {
  ...
  if (!apiBase) {
    ...
    setState(
      ...
      output,
      "idle",
      "Envio em preparação. Nenhum dado será transmitido nesta etapa.",
    );
    return;
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!form.reportValidity() || form.dataset.state === "loading") {
      return;
    }
    ...
    try {
      ...
      const successRedirect =
        form.dataset.successRedirect?.trim();
